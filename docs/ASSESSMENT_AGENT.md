...
| **A3 — Frontend** ✅ | Ask/Assess nav rail; intake form (description + clarify toggle + sample); live stage timeline with per-stage status and running counters over the SSE; clarification panel; report view (executive summary, at-a-glance stats, obligation cards with gap-status badges + citations, remediation roadmap with priority/effort/tradeoffs, profile, authenticated `.md` download); assessments list with status chips + delete. Live-verified via Playwright end-to-end | done |
| **A4 — Evals & hardening** ✅ | Scenario suite (`evals.cli assessment`) running the real engine over 28 scenarios incl. 3 prompt-injection red-team descriptions; **two-tier safety gate** — hard gate: a prohibited practice is never cleared as `does_not_apply` when it applies (false-clear rate = 0; `needs_info` is a safe flag, not a clear); quality gates: verdict accuracy ≥ 0.9, positive blocker recall ≥ 0.9, injection resistance ≥ 0.9. Hardening: the prohibited-practice batch is classified by the judge-tier model (the cheap model confidently misread untargeted scraping), profile extraction now captures data-sourcing, lenient JSON repair. Per-assessment Prometheus metrics + Grafana panels (outcomes, cost/assessment). Wired into the `Evals` CI workflow. Live-verified over 28 scenarios: blocker recall 9/9, 0 false-clears, injection resistance 0.91, verdict accuracy 0.88, ~$0.016/assessment | done |
